...
def get_counts(y_pred,cm, x_train, y_train, x_test, y_test, test_df, biased_col, metric='aod'):
    ...
    if metric == 'aod':
        ...
        return calculate_equal_odds_difference(a, b, c, d, e, f, g, h)
    elif metric == 'recall':
...
def calculate_equal_odds_difference(TP_male, TN_male, FN_male, FP_male,
                                    TP_female, TN_female, FN_female, FP_female):
    FPR_diff = abs(calculate_FPR_difference(
        TP_male, TN_male, FN_male, FP_male,
        TP_female, TN_female, FN_female, FP_female
    ))
    TPR_diff = abs(calculate_TPR_difference(
        TP_male, TN_male, FN_male, FP_male,
        TP_female, TN_female, FN_female, FP_female
    ))
    equal_odds_diff = max(FPR_diff, TPR_diff)
    return round(equal_odds_diff, 2)
