...
from Baseline.Generate_Samples import generate_samples
...
def Fair_Smote(df, base_clf, keyword, rep=10):
    ...
    for i in range(rep):
        start = time.time()
        dataset_orig_train, dataset_orig_test = train_test_split(dataset_orig, test_size=0.2, random_state=42)
        ...
        eopd.append(measure_final_score(dataset_orig_test, y_pred, cm, X_train, y_train, X_test, y_test, keyword, 'eopd'))
        print('Time', time.time() - start)
    res1 = [acc, pre, recall, f1, aod1, eod1, spd1, di1, fr, eopd]
    return res1
